...
expo_host="${DINNER_SWIPE_EXPO_HOST:-localhost}"
...
tmux new -d -s "$session_name" \
  "cd '$app_dir' && : > '$log_file' && set -a && [ -f '$expo_env_file' ] && . '$expo_env_file'; set +a; EXPO_PUBLIC_API_URL='$api_url' npx expo start --dev-client --host '$expo_host' --clear --port 8081 2>&1 | tee -a '$log_file'"

echo "Started Expo dev-client Metro in tmux session: $session_name"
echo "Attach with: tmux attach -t $session_name"
echo "Logs: $log_file"
echo "API URL: $api_url"
echo "Expo host: $expo_host"
