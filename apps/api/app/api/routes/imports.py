...
@router.post("/upload")
async def upload(file: UploadFile, db: DbDep, current_user: BasicUser) -> dict[str, object]:
    return await save_upload(db, current_user, file)


@router.post("/{batch_id}/mapping")
def map_columns(
    batch_id: str, payload: ImportMappingRequest, db: DbDep, current_user: BasicUser
) -> dict[str, object]:
    return validate_batch(db, current_user, batch_id, payload)


@router.get("/{batch_id}/preview")
def preview(batch_id: str, db: DbDep, current_user: BasicUser) -> dict[str, object]:
    return preview_batch(db, current_user, batch_id)


@router.post("/{batch_id}/confirm")
def confirm(batch_id: str, db: DbDep, current_user: BasicUser) -> dict[str, object]:
    return confirm_batch(db, current_user, batch_id)


@router.get("/{batch_id}/errors.csv")
def errors_csv(batch_id: str, db: DbDep, current_user: BasicUser) -> Response:
    return Response(
